...
const listInterrogations = async () => {
  try {
    ...
    const interrogations = await Interrogation.find().populate('createdBy', 'username email');
    ...
    interrogations.forEach(interrogation => {
      ...
      console.log(`Notes: ${interrogation.notes}`);
      if (interrogation.createdBy) {
        // @ts-ignore
        console.log(`Created By: ${interrogation.createdBy.username} (${interrogation.createdBy.email})`);
      }
      // @ts-ignore
